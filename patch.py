...
def apply_patch():
    with open('MockTab/Driver/Injection/InputInjector+PenInjection.swift', 'r') as f:
        content = f.read()

    new_content = """    func inject(point: TabletPoint, settings: TabletSettings?) {
        rearmWatchdog()
...
"""
    ...
    # Use regular expression to find the function inject to replace.
    # Match the definition until the next method (private func commitProximityExit)
    import re
    pattern = re.compile(r'    func inject\(point: TabletPoint, settings: TabletSettings\?\) \{.*?(?=    /// Runs the proximity-exit cleanup:)', re.DOTALL)

    res = pattern.sub(new_content, content)

    with open('MockTab/Driver/Injection/InputInjector+PenInjection.swift', 'w') as f:
        f.write(res)
