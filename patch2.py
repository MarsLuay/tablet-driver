...
"""
    ...
    start_str = "    func inject(point: TabletPoint, settings: TabletSettings?) {"
    end_str = "    /// Runs the proximity-exit cleanup: releases the tip, any held USB/middle"

    start_idx = content.find(start_str)
    end_idx = content.find(end_str)

    if start_idx != -1 and end_idx != -1:
        res = content[:start_idx] + new_content + content[end_idx:]
